...
exports.githubLink = exports.issueBranchPattern = exports.searchIssueTagBranch = exports.requestBranch = undefined;
...
const githubLink = exports.githubLink = async branch => {
  const remoteUrl = (await (0, _promise2.default)().getRemotes(true)).find(remote => remote.name === _config2.default.remote).refs.fetch;
  const nameRegexp = /.*:(.*\/.*).git/;

  if (remoteUrl.match(nameRegexp)) {
    const name = remoteUrl.match(nameRegexp)[1].trim();

    return `https://github.com/${name}/compare/${_config2.default.mainBranch}...${branch}?expand=1`;
  }
};